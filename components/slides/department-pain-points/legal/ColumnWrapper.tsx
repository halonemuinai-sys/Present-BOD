...
import type { LegalColumn } from "./data";
...
export default function ColumnWrapper({ data, children }: { data: LegalColumn; children: ReactNode }) {
  return (
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div>
          <p className="text-sm font-black uppercase leading-tight text-emerald-800 md:text-base">
            {data.title} <span className="text-slate-400">{data.subtitle}</span>
          </p>
        </div>
      </div>
