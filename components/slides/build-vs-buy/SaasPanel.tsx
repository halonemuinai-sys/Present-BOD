...
export default function SaasPanel() {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="flex flex-1 flex-col rounded-2xl border border-slate-200 bg-white p-6"
    >
      <div className="mb-2 flex items-center justify-between">
        <span className="text-xs font-bold uppercase tracking-widest text-amber-700">
          {saasPanel.label}
        </span>
        <span className="text-xs text-slate-400">{saasPanel.note}</span>
      </div>
      ...
      <motion.div variants={container} initial="hidden" whileInView="visible" viewport={{ once: true }}>
        {saasPanel.items.map((item, i) => (
          <motion.div
            key={item.title}
            variants={row}
            className={`flex items-start justify-between gap-4 py-3 ${i > 0 ? "border-t border-slate-100" : ""}`}
          >
            <div>
